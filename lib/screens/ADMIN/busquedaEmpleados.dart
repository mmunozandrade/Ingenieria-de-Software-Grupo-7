...
class _BusquedaEmpleadosState extends State<BusquedaEmpleados> {
  ...
  Future<void> _buscar() async {
    ...
    if (_tipoBusq == 'apellido' && query.length < 3) {
      setState(
        () => _error = 'Ingresa al menos 3 caracteres para buscar por apellido',
      );
      return;
    }
    ...
    try {
      ...
      final uri = Uri.parse(
        '$_apiUrlBusq/buscar-empleados',
      ).replace(queryParameters: {_tipoBusq: query});
  ...
  void _abrirFicha(Map<String, dynamic> empleado) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => FichaEmpleadoAdmin(empleado: empleado)),
    ).then((_) {
                      ...
                      Container(
                        ...
                        child: const Column(
                          children: [
                            ...
                            SizedBox(height: 4),
                            Text(
                              'Intenta con otro apellido o RUT',
                              style: TextStyle(
                                fontSize: 13,
                                color: Color(0xFF94A3B8),
                              ),
