...
class _AprobarSolicitudesVState extends State<AprobarSolicitudesV> {
  ...
  Future<void> _cargarSolicitudes() async {
    ...
    try {
      ...
      final uri = Uri.parse(
        '$_apiUrlAprob/admin/solicitudes-vacaciones',
  ...
  Future<void> _decidir(Map<String, dynamic> s, String decision) async {
    ...
    if (decision == 'Rechazada') {
      if (observacion.length < 10) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'La observacion es obligatoria al rechazar (minimo 10 caracteres)',
            ),
    ...
    try {
      ...
      final response = await http.put(
        ...
        body: jsonEncode({'estado': decision, 'observacion': observacion}),
      );
