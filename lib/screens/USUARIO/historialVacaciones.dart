...
class _HistorialVacacionesState extends State<HistorialVacaciones> {
  ...
  Future<void> _cargarHistorial() async {
    ...
    try {
      ...
      final uri = Uri.parse(
        '$_apiUrlHist/mis-solicitudes-vacaciones',
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'El sistema no pudo generar el comprobante. Verifique que todos los datos esten completos',
              ),
                                  ...
                                  children: [
                                    ...
                                    Wrap(
                                      ...
                                      children: [
                                        _ItemHistorial(
                                          label: 'Dias habiles',
                                          valor: '${s['dias_habiles']}',
                                        ),
                                        _ItemHistorial(
                                          label: 'Fecha solicitud',
                                          valor: s['fecha_solicitud'] ?? '—',
                                        ),
